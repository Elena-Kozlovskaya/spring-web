...
public class PayPalService {
    ...
    public OrderRequest createOrderRequest(Long orderId) {
        com.geekbrains.spring.web.core.entities.Order order = orderService.findByIdWithStatus(orderId, OrderStatus.NOT_PAID.name()).orElseThrow(() -> new ResourceNotFoundException("Заказ не найден"));
        ...
        PurchaseUnitRequest purchaseUnitRequest = new PurchaseUnitRequest()
                ...
                .shippingDetail(new ShippingDetail().name(new Name().fullName(order.getUsername()))
                        .addressPortable(new AddressPortable().addressLine1(order.getAddress()).addressLine2("Floor 6")
                                .adminArea2(order.getCity()).adminArea1("CA").postalCode(order.getPostalCode()).countryCode("US")));
